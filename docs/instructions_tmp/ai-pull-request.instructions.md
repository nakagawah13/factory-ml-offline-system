...
| 種別 | 対象 | 結果 |
|------|------|------|
| ユニットテスト | pytest (27件) | ❌ Fail (1件失敗) |
| Lint | ruff check | ✅ Pass |

### 失敗の詳細

**test_validator.py::test_empty_dataframe**:
- 原因: 空のDataFrameに対するバリデーションで予期しないNoneが返される
- 次アクション: 空チェックロジックを修正（次コミットで対応予定）

その他の失敗詳細は `pytest.log` を参照してください。
```
...
git rebase origin/main  # コンフリクトがあれば解決
```
...
```markdown
# feat: add data validation for input CSV files

## 変更概要

- `DataValidator` クラスを新規実装
- スキーマ定義ファイル (`config/schema.json`) を追加
- `DataLoader` にバリデーション統合
- ユニットテスト15件を追加

## 変更理由

不正なデータが推論パイプラインに流入し、モデル精度低下の原因となっていた。
入力段階でのデータ検証により、異常を早期検出する。
...
|---------|------|----------------------|------|
| T-001 | データ品質 | 🔵 In Progress → ✅ Done | DataValidatorクラス実装 |
| T-002 | データ品質 | ⚪ Not Started → ✅ Done | スキーマ定義作成 |
...
### 依存モジュール
- `preprocessor.py` - 動作確認済み
- `model_trainer.py` - 動作確認済み
...
- [ ] mainとの差分が意図通りか確認 (`git diff main`)
- [ ] コンフリクトがないか確認
- [ ] Lint/テストが通過
- [ ] 必須セクションがすべて記載されている
  - [ ] 変更概要
...
| 変更理由 | **必須** | なぜ変更したか |
| タスク対応 | **必須** | 計画との整合性 |
| 影響範囲 | **必須** | 影響を受けるファイル・モジュール |
| テスト / 検証 | **必須** | テスト結果 |
| リスク / 互換性 | **必須** | 破壊的変更の有無 |
| 関連Issue / PR | 任意 | 参照情報 |
| 実験結果 | **必須**（ML/AI） | モデル評価指標 |
