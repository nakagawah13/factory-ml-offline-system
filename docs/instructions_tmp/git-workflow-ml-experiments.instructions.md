---
applyTo: "**"
---

# Git Workflow Instructions - ML/AI Experiments

このガイドラインは、ML/AIプロジェクトにおける**実験管理とバージョニング**に特化したAI向けインストラクションです。

## このファイルの役割

このインストラクションは以下の内容を含みます:

- **実験管理**: Gitタグによる実験マイルストーンの記録
- **再現性確保**: 乱数シード、依存関係、実験設定、環境設定の管理
- **実験レビュー**: ML/AI特有のレビュー観点

## 関連ガイドライン

- **基本Gitワークフロー**: [git-workflow.instructions.md](./git-workflow.instructions.md)で定義
- **プロジェクト構造**: [ai-project-structure-ml.instructions.md](../../.github/instructions/ai-project-structure-ml.instructions.md)で定義

---

## 実験管理とバージョニング

### Gitタグによる実験マイルストーンの記録

重要な実験結果や本番デプロイ時には、**Gitタグ**を使用してバージョンを記録します。

#### タグの種類

**軽量タグ（Lightweight Tag）**:
- 単純なポインタ
- 実験の中間チェックポイントに使用

```bash
# 軽量タグの作成
git tag exp-v1.2.3
```

**注釈付きタグ（Annotated Tag）**:
- 完全なGitオブジェクト
- 作成者、日付、メッセージを含む
- **重要なマイルストーンに推奨**

```bash
# 注釈付きタグの作成
git tag -a v1.0.0 -m "Production release: baseline model with 85% accuracy"
git tag -a exp-baseline -m "Baseline experiment: LightGBM with default params"
```

#### タグ命名規則

**本番リリース**:
```
v<major>.<minor>.<patch>
例: v1.0.0, v1.2.3, v2.0.0
```

**実験マイルストーン**:
```
exp-<experiment-name>-v<version>
例: exp-baseline-v1, exp-feature-engineering-v2
```

**モデルバージョン**:
```
model-<model-name>-v<version>
例: model-lightgbm-v1.2, model-xgboost-v2.0
```

#### タグ付けの実践例

```bash
# 実験が成功した場合
git tag -a exp-baseline-v1 -m "Baseline experiment

Model: LightGBM
Accuracy: 85.2%
Precision: 83.1%
Recall: 87.3%
Dataset: 2025-12-01 to 2025-12-05
Seed: 42"

# タグをリモートにプッシュ
git push origin exp-baseline-v1

# すべてのタグをプッシュ
git push origin --tags
```

#### タグの確認

```bash
# すべてのタグを表示
git tag

# パターンマッチでタグを検索
git tag -l "exp-*"
git tag -l "v1.*"

# タグの詳細情報を表示
git show exp-baseline-v1
```

#### タグの削除

```bash
# ローカルのタグを削除
git tag -d exp-test-v1

# リモートのタグを削除
git push origin --delete exp-test-v1
```

---

## 実験の再現性確保

ML/AIプロジェクトでは、実験の再現性が極めて重要です。以下の要素を必ず記録・管理してください。

### 1. 乱数シードの固定

すべての乱数生成で同じシードを使用します。

**Python例**:
```python
import random
import numpy as np
import lightgbm as lgb

# 乱数シードを固定
SEED = 42

random.seed(SEED)
np.random.seed(SEED)

# モデル訓練時にもシードを指定
model = lgb.LGBMClassifier(random_state=SEED)
```

**設定ファイルでの管理**:
```json
{
  "experiment": {
    "random_seed": 42,
    "description": "固定シードで再現性を確保"
  }
}
```

### 2. 依存関係の明示

すべてのライブラリバージョンを明確に記録します。

**requirements.txtの生成**:
```bash
# 現在の環境のパッケージを出力
pip freeze > requirements.txt

# または、主要なパッケージのみ記載（推奨）
# requirements.txt
lightgbm==4.0.0
pandas==2.1.0
numpy==1.24.3
scikit-learn==1.3.0
onnx==1.14.0
onnxruntime==1.15.0
```

**pyproject.tomlでの管理**:
```toml
[project]
dependencies = [
    "lightgbm>=4.0.0,<5.0.0",
    "pandas>=2.1.0,<3.0.0",
    "numpy>=1.24.0,<2.0.0",
]
```

### 3. 実験設定の記録

実験ごとに設定ファイルを保存します。

**実験設定例（config/experiment_baseline.json）**:
```json
{
  "experiment_id": "exp-baseline-v1",
  "model": {
    "type": "lightgbm",
    "params": {
      "n_estimators": 100,
      "learning_rate": 0.1,
      "max_depth": 5,
      "random_state": 42
    }
  },
  "data": {
    "train_start": "2025-12-01",
    "train_end": "2025-12-05",
    "test_split": 0.2
  },
  "features": [
    "temperature",
    "pressure",
    "humidity",
    "lag_1",
    "lag_2"
  ]
}
```

**Gitで設定ファイルをバージョン管理**:
```bash
git add config/experiment_baseline.json
git commit -m "exp: add baseline experiment configuration"
git tag -a exp-baseline-v1 -m "Baseline experiment configuration"
```

### 4. 環境設定の記録

実行環境の情報を記録します。

**環境情報の記録例**:
```bash
# Pythonバージョン
python --version > environment.txt

# インストール済みパッケージ
pip list >> environment.txt

# システム情報
uname -a >> environment.txt
```

**Dockerfile（コンテナ環境）**:
```dockerfile
FROM python:3.11-slim

# 依存関係をインストール
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# 環境変数を設定
ENV PYTHONUNBUFFERED=1
ENV RANDOM_SEED=42
```

### 再現性チェックリスト

実験を実施する際は、以下を確認してください:

- [ ] 乱数シードが固定されている
- [ ] requirements.txtまたはpyproject.tomlが最新
- [ ] 実験設定ファイルがGitで管理されている
- [ ] データの取得方法がドキュメント化されている
- [ ] 環境変数が記録されている
- [ ] 実験結果がログファイルに記録されている
- [ ] 重要なマイルストーンにGitタグが付けられている

---

## 実験レビューのガイドライン

ML/AIプロジェクトのPRでは、実験レビューが重要です。

### 実験レビューのチェックポイント

**1. 実験設計**:
- 実験の目的と仮説が明確か
- ベースラインとの比較が適切か
- 実験変数が適切に制御されているか

**2. 評価指標**:
- 適切な評価指標が選択されているか
- 評価指標がビジネス目標と整合しているか
- 複数の指標で多角的に評価しているか

**3. ベースライン比較**:
- 既存モデルとの性能比較があるか
- 統計的有意性が検証されているか
- 改善幅が実用レベルか

**4. 結果の妥当性**:
- 結果が統計的に有意か
- 過学習の兆候がないか（train/validation差）
- 外れ値や異常値の影響を確認しているか

**5. 過学習チェック**:
- train/validationの性能差は適切か
- クロスバリデーション結果は安定しているか
- テストデータでの性能低下がないか

**6. 実験記録**:
- 実験条件と結果が記録されているか
- 失敗した実験も記録されているか
- 考察と次のアクションが明確か

### 実験レビューのベストプラクティス

**レビュアー向け**:

```markdown
# 実験結果の確認
✅ ベースラインより3%精度向上を確認しました
📊 confusion matrixを見ると、false positiveが減少しています
❓ validation accuracyが85%でtrain accuracyが95%ですが、過学習の懸念はありませんか？
```

**PR作成者向け**:

実験結果を含むPRには、以下を記載してください:

```markdown
## 実験結果

### 目的
不良品検出精度の向上

### 変更内容
- 特徴量エンジニアリング: lag特徴量を追加
- ハイパーパラメータチューニング: learning_rate 0.1 → 0.05

### 結果
| モデル | Accuracy | Precision | Recall | F1 |
|--------|----------|-----------|--------|-----|
| Baseline | 82.3% | 80.1% | 84.2% | 82.1% |
| Improved | 85.5% | 83.4% | 87.8% | 85.5% |

### 考察
- lag特徴量が時系列パターンを捉え、精度向上に寄与
- false positiveが減少し、実運用での誤報が削減される見込み
- 次のステップ: より長期のlagを検証

### 再現性
- Seed: 42
- Dataset: 2025-12-01 to 2025-12-05
- Tag: exp-lag-features-v1
```

---

## まとめ

### 重要なポイントの再確認

1. **タグ付け**: 重要な実験にはGitタグを付ける
2. **再現性**: 乱数シード、依存関係、設定を必ず記録
3. **実験記録**: 成功・失敗に関わらず記録を残す
4. **レビュー**: 実験設計、結果、妥当性を多角的に確認

### クイックリファレンス

```bash
# 実験用タグの作成
git tag -a exp-baseline-v1 -m "Baseline experiment: accuracy 85%"
git push origin exp-baseline-v1

# 実験設定のコミット
git add config/experiment_*.json
git commit -m "exp: add baseline experiment configuration"

# 依存関係の記録
pip freeze > requirements.txt
git add requirements.txt
git commit -m "chore: update dependencies for experiment"
```
